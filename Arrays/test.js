function diagonalDifference(arr) {
    ...
    for (let i = 0; i < arr.length; i += 1) {
        ...
        secondDiagonalSum += arr[i][arrLength - 1 - i]
    }
    // console.log(firstDiagonalSum)
    // console.log(secondDiagonalSum)
    // console.log(Math.abs(firstDiagonalSum - secondDiagonalSum))
...
function staircase(n) {
    ...
    for (let i = 0; i < n; i += 1) {
        let tempArr = ""
        for (let y = 1; y < n - i; y += 1) {
            tempArr += " "
        }
        for (let z = n - i; z <= n; z += 1) {
            tempArr += "#"
        }
        matrix.push(tempArr)
    }
...
function hourglassSum(arr) {
    ...
    for (let i = 0; i < arr.length - 2; i += 1) {
        for (let y = 0; y < arr.length - 2; y += 1) {
            let tempSum = arr[i][y] + arr[i][y + 1] + arr[i][y + 2] + arr[i + 1][y + 1] + arr[i + 2][y] + arr[i + 2][y + 1] + arr[i + 2][y + 2]
            if (tempSum > maxSum) {
                maxSum = tempSum
            }
        }
